  ...
  describe '#create_row_moves' do
    ...
    context 'when the column of a square is out of bounds (not between 0 & 7)' do
      it 'stops loop and returns empty array' do
        square = [3, 10]
        left_move = [0, -1]
        create_moves = rook.create_row_moves(square, left_move, rook_board)
    ...
    context 'when given the square [5, 6] with the left movement' do
      before do
        allow(rook).to receive(:occupied_by_opponent?).and_return(false)
        allow(rook).to receive(:occupied_by_own_self?).and_return(false)
      end
    ...
    context 'when given the square [5, 6] with the right movement' do
      ...
      it "returns 1 valid square ([5, 7])" do
        ...
        right_move = [0, 1]
        valid_moves = [[5, 7]]
        create_moves = rook.create_row_moves(square, right_move, rook_board)
        expect(create_moves).to eq(valid_moves)
      end
    end
    ...
    context 'when given square [0, 0] with right movement and square [0, 2] is occupied by own self' do
      ...
      before do
        allow(rook_board).to receive(:occupied_by_opponent?).and_return(false)
        allow(rook_board).to receive(:occupied_by_own_self?).and_return(true)
    ...
    context 'when given square [2, 5] with left movement and square [2, 1] is occupied by opponent' do
      ...
      before do
        allow(rook_board).to receive(:occupied_by_opponent?).and_return(true)
        allow(rook_board).to receive(:occupied_by_own_self?).and_return(false)
        allow(rook_board).to receive(:board=)
  ...
  describe '#create_column_moves' do
    ...
    context 'when the row of a square is out of bounds (not between 0 & 7)' do
      it 'stops loop and returns empty array' do
        square = [9, 1]
        top_move = [-1, 0]
        create_moves = rook.create_column_moves(square, top_move, rook_board)
        expect(create_moves).to eq([])
      end
    end
    ...
    context 'when given the square [3, 3] with the bottom movement' do
      ...
      it "returns 4 valid squares ([4, 3], [5, 3], [6, 3], [7, 3])" do
        ...
        create_moves = rook.create_column_moves(square, bottom_move, rook_board)
        expect(create_moves).to eq(valid_moves)
      end
    end
